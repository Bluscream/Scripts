# Publish-Module.psm1
# PowerShell module containing all build and publish functions
# This eliminates the need for complex parameter passing between scripts
...
"@
    ...
    foreach ($dockerfile in $dockerfiles) {
        ...
        foreach ($config in $buildConfigs) {
            ...
            # Build Docker image for Docker Hub - use lowercase for Docker compliance
            $dockerImageName = "$($projectName.ToLower())$configTag"
...
function Publish-GitHubRelease {
    ...
    foreach ($result in $results) {
        if ($result.ExitCode -eq 0) {
            Write-Host "Successfully uploaded: $($result.Name)"
        }
        else {
...
function Publish-DockerHub {
    ...
    foreach ($config in $buildConfigs) {
        $configTag = if ($config -eq "Release") { "release" } else { "debug" }
        $localImageName = "$($projectName.ToLower())$configTag"
...
function Build-Project {
    ...
    foreach ($key in $allParams.Keys) {
        if (-not $PSBoundParameters.ContainsKey($key)) {
            Write-Host (" - {0}: {1}" -f $key, $allParams[$key])
        }
    }
    ...
    foreach ($csproj in $csprojFiles) {
        ...
        if ($Docker) {
            Build-DockerImages -projectDir $projectDir -projectName $projectName -newVersion $newVersion -buildConfigs $buildConfigs
        }
...
function Publish-Project {
    ...
    if ($publishSuccess) {
        Write-Host "All publishing operations completed successfully!"
    }
    else {
...
# Export functions
Export-ModuleMember -Function Get-Username, Bump-Version, Set-Version, Kill-ProcessesByName, Commit-Git, Push-Git, Build-DockerImages, Publish-GitHubRelease, Publish-NuGet, Publish-DockerHub, Publish-GHCR, Build-Project, Publish-Project 